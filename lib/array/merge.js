...
var markConflicts = function(updates, conflicts) {
...
var union = function(diff1, diff2) {
  var result = []
  while(diff1.length && diff2.length) {
    ...
    if (newIndexes[0] == newIndexes[1]) {
      result.push([ entries[0][0], {multiple: [entries[0][1], entries[1][1]]} ])
...
var diffEntries = function(updates1, updates2, conflicts) {
...
var mergeDiffs = function(diffs, conflicts) {
  diffs = diffs.map(sortByNewIndexDesc)
  var diff1 = diffs[0], diff2 = diffs[1]
  var diffUnion = union(diff1, diff2)
  return diffUnion.map(function(entry) {
    var newIndex = entry[0], updates = entry[1]
    if (updates.multiple) {
      return [newIndex, mergeEntry(updates.multiple[0], updates.multiple[1], conflicts)]
    } else {
      return [newIndex, markConflicts(updates, conflicts)]
    }
  })
}
...
var merge = function(diff1, diff2) {
  ...
  var mergedMoveMaps = objectMerge(moveMaps)
  var conflicts = new Conflicts(mergedMoveMaps.conflict)
  var result = {diff: mergeDiffs(diffs, conflicts)}
