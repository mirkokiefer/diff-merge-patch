...
var conflictResolution = {
  pickIndex: function(result, index) {
    result = _.clone(result)
    if (result.move) { result.move = result.move[index] }
    if (result.delete) { result.delete = result.delete[index] }
    return result
  },
  // pick the index with the lowest hash:
  commutative: function(result) {
    if (!result.move && !result.delete) return result
    var arrays
    if (result.move && result.delete) {
      arrays = result.move.map(function(each, i) { return each.concat(result.delete[i]) })
    } else if (result.move) {
      arrays = result.move
    } else if (result.delete) {
      arrays = result.delete
    } else {
      return result
    }
    var hashs = arrays
    ...
    return conflictResolution.pickIndex(result, index)
  }
}

merge.resolveConflicts = conflictResolution
